...
def render_studio():
    ...
    .canvas-container { display: grid; grid-template-columns: repeat(auto-fit, minmax(140px, 1fr)); gap: 10px; margin-bottom: 15px; width: 100%; }
    ...
    /* Pure White Background & Dynamic Bold Solid Black Text Overriding Engine */
    ...
    .stDownloadButton>button {
        ...
        box-shadow: 0px 4px 12px rgba(255, 255, 255, 0.2) !important;
    ...
    .legal-box { background: #161b26; border: 1px solid #2d3748; border-radius: 8px; padding: 15px; margin-top: 10px; }
    ...
    option = st.sidebar.radio("CHOOSE ACTIVE ENGINE:", ["AI VOICE CHANGER", "AUDIO NOISE CLEANER", "VIDEO TO MP3 CONVERTER", "SMART VIDEO CUTTER", "AI VIDEO SPEED CONTROLLER", "AI AUDIO BASS BOOSTER"])
    ...
    tools_list = [("Voice Changer", "Shift pitch easily.", "AI VOICE CHANGER"), ("Noise Cleaner", "Remove audio hiss.", "AUDIO NOISE CLEANER"), ("Video To MP3", "Extract clear track.", "VIDEO TO MP3 CONVERTER"), ("Video Cutter", "Trim clips fast.", "SMART VIDEO CUTTER"), ("Speed Control", "Slow-mo & Fast.", "AI VIDEO SPEED CONTROLLER"), ("Bass Booster", "Boost frequencies.", "AI AUDIO BASS BOOSTER")]
    for title, desc, key in tools_list:
    ...
    if legal_option == "ABOUT US": st.markdown('<div class="legal-box"><h4>About Our Studio</h4><p>Welcome to AI Smart Media Studio! We provide high-performance, web-based tools designed to transform your video and audio streams seamlessly.</p></div>', unsafe_allow_html=True)
    elif legal_option == "PRIVACY POLICY": st.markdown('<div class="legal-box"><h4>Privacy Policy</h4><p>Your data safety is our priority. Files are processed inside isolated temp directories and permanently purged after downloads.</p></div>', unsafe_allow_html=True)
    elif legal_option == "COOKIES & TERMS": st.markdown('<div class="legal-box"><h4>Cookies & Terms</h4><p>This application uses local session variables and cookies required by ad networks like Google AdSense.</p></div>', unsafe_allow_html=True)
    else: core_engines.run_workspace(option)
